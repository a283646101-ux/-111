...
public class RoomServiceImpl implements RoomService {
    ...
    public List<RoomVo> getRoomsWithDeviceByFloorId(Long floorId) {
        ...
        roomVos.forEach(roomVo -> {
            ...
            if (CollUtil.isNotEmpty(roomDeviceVos)) {
                roomDeviceVos.forEach(this::fillLastDataFromRedis);
            }
            ...
            if (CollUtil.isNotEmpty(bedVoList)) {
                bedVoList.forEach(bedVo -> {
                    ...
                    if (CollUtil.isNotEmpty(bedDeviceVos)) {
                        bedDeviceVos.forEach(this::fillLastDataFromRedis);
                    }
                });
            }
        });
    ...
    private void fillLastDataFromRedis(DeviceInfo deviceInfo) {
        if (ObjectUtil.isEmpty(deviceInfo) || ObjectUtil.isEmpty(deviceInfo.getIotId())) {
            return;
        }
        Object jsonObj = redisTemplate.opsForHash().get(CacheConstants.IOT_DEVICE_LAST_DATA, deviceInfo.getIotId());
